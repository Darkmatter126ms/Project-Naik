...
def main() -> int:
    ...
    # --- persona sweep: every quote is valid and differentiated ---------------
    ...
    for inp, _gap in persona_suite():
        ...
        quotes.append((inp, r))
        print(f"  {inp.user_id:8s} {inp.kecamatan:14s} gig={str(inp.is_gig_worker):5s} "
              f"premium Rp {r.weekly_premium_idr:>7,}/wk  payout Rp {r.quote.payout_per_event_idr:>10,}"
              f"  {r.quote.payout_multiple:.0f}d")
    check("every persona yields a valid quote with premium < payout",
          all(r.quote.premium_idr < r.quote.payout_per_event_idr for _, r in quotes))
    check("every persona's product is complementary to SiProPer",
          all("siproper" in r.bahasa_description.lower() for _, r in quotes))

    # Flood differentiation: Sari (Penjaringan 0.92) pays more than a low-flood
    # persona of similar income/gig status, if present in the suite.
    sari_prem = next(r.weekly_premium_idr for inp, r in quotes if inp.user_id == "sari")
    low_flood = [r.weekly_premium_idr for inp, r in quotes if inp.kecamatan in ("Cilandak", "Menteng")]
    if low_flood:
